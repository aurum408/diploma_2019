...
import torch
from utils.visualize import VisdomPlotter
import os
import pdb
...
class Logger(object):
    ...
    def _unnorm(self, arr, mean, std):
        n = arr.shape[0]
        arr_n = np.stack([np.stack([arr[i][j, :, :]*std[j]+mean[j] for j in range(3)], axis=0) for i in range(n)],
                               axis=0)
        return arr_n
    ...
    def draw(self, right_images, fake_images):
        ...
        fake_img = fake_images.data.cpu().numpy()
        right_img = right_images.data.cpu().numpy()
        fake_img = self._unnorm(fake_img, mean, std)
        right_img = self._unnorm(right_img, mean, std)

        self.viz.draw('generated images', fake_img[:64])
        self.viz.draw('real images', right_img[:64])


def load_loss(logfpath):
    with open(logfpath, "r") as fp:
        lines = fp.readlines()
    fp.close()
    ...
    for line in lines:
        for item in line[1:]:
            name, val = item.split("= ")
            val = float(val)
            if name in loss_dct.keys():
                loss_dct[name].append(val)
            else:
                loss_dct.update({name:[val]})

    return loss_dct


if __name__ == '__main__':
    fpath = "/hdd1/diploma_2019/log2.txt"
    loss = load_loss(fpath)
    print("ok")
